var Template = {
	...
	initGoogleMap: function(){
		...
		var mapCfg = this.googleMapConfig || {},
			markerPositionCfg = jQuery.extend({lat:-33.9500,lng:-83.3833}, mapCfg.markerLocation || {}),
	...
	_initAjaxPagesLoading: function(el, selector){
		if (!el || this.menu.isFileProtocol()) return;
...
Template.menu = {
	...
	init:function(config){
		...
		$(window).on('hashchange', function(event){
			// to prevent page reloading for any anchor
			if (event.fragment && !event.fragment.match(/\.html$/)) {
				return;
			}
